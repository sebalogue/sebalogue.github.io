        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            if (textureColor.z > 2.0*textureColor.x && textureColor.z > 2.0*textureColor.y) {
                position += normal*(1.0+cos(uv.x*26.0*PI+time*60.0))*0.012;
                position += normal*(1.0+cos(uv.y*26.0*PI+time*60.0))*0.012;
            } else {
                position += normal*0.1;
            }
